...
def send_order_confirmation_email(order):
    ...
    customer = order.customer
    restaurant = order.restaurant
    subject = f"Bestellbestätigung - {restaurant.resturent_name}"
    ...
    message = format_html(
        ...
        <p>Hallo <b>{}</b>,</p>
        <p>Vielen Dank für Ihre Bestellung bei <b>{}</b>. Hier sind Ihre Bestelldetails:</p>
        ...
        <table border="1" cellpadding="6" cellspacing="0" style="border-collapse: collapse;">
            ...
            {}
            <tr>
                <td colspan="6" align="left"><b>Liefergebühr</b></td>
                <td><b>{}</b></td>
            </tr>
            <tr>
                <td colspan="6" align="left"><b>Gesamtpreis</b></td>
                <td><b>{}</b></td>
            </tr>
        </table>
        ...
        <p><b>Adresse:</b> {}</p>
        <hr>
        <p>Wir freuen uns, Sie bedienen zu dürfen!</p>
        """,
        customer.customer_name,
        restaurant.resturent_name,
        item_rows,
...
def send_order_verified_email(order):
    ...
    subject = f"✅ Bestellung #{order.id} – Verifizierung erforderlich"

    message = f"""
    Hallo {customer.customer_name},

    Ihre Bestellung (ID: {order.id}) wurde erfolgreich erstellt.

    Bitte klicken Sie auf den folgenden Link, um Ihre Bestellung zu bestätigen:
    {verify_link}

    Vielen Dank, dass Sie {restaurant.resturent_name} gewählt haben!

    🧾 Bestellübersicht:
    - Gesamtpreis: ${order.total_price}
    - Adresse: {customer.address or 'N/A'}

    Wir schätzen Ihr Vertrauen und freuen uns, Sie bald wieder bedienen zu dürfen!

    Mit freundlichen Grüßen  
    Ihr {restaurant.resturent_name} Team
    """
    ...
    <html>
    <head>
        <title>Bestellverifizierung</title>
        <style>
            body {{
                font-family: Arial, sans-serif;
                background-color: #f8f9fa;
    ...
    <body>
        <div class="container">
            <h2>🔔 Bestellverifizierung erforderlich</h2>
            <p>Hallo <b>{customer.customer_name}</b>,</p>
            <p>Ihre Bestellung (<b>ID: {order.id}</b>) wurde erfolgreich erstellt.</p>
            <p>Bitte bestätigen Sie Ihre Bestellung, indem Sie auf die Schaltfläche unten klicken:</p>
            <p style="text-align: center;">
                <a href="{verify_link}" class="verify-btn">Meine Bestellung bestätigen</a>
            </p>

            <h3>🧾 Bestellübersicht</h3>
            <p><b>Gesamtpreis:</b> ${order.total_price}</p>
            <p><b>Adresse:</b> {customer.address or "N/A"}</p>
